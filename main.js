...
const MOCK_DB = [
  {
    ...
    summary:'VIP 행사에 최적화된 프리미엄 연회 공간입니다. 한강 조망과 4K 레이저 AV 시스템, 상주 케이터링팀이 완벽한 행사를 지원합니다.',
    tags:['vip','주차여유','품격','특급','강남외'],
    pros:[
      {icon:'✦', title:'VIP 전용 동선', desc:'전용 엔트런스와 대기실로 임원급 의전에 최적입니다.'},
      {icon:'✦', title:'프리미엄 AV', desc:'4K 레이저 프로젝터 3대 + 상주 AV 엔지니어 제공됩니다.'},
  ...
  },
  {
    ...
    summary:'국내 최대 도심형 컨벤션 시설로 삼성역 직결 접근성이 뛰어납니다. 유연한 홀 분할 구성과 다양한 케이터링 파트너를 보유합니다.',
    tags:['접근성','대형','컨벤션','강남'],
    pros:[
      {icon:'✦', title:'삼성역 직결', desc:'지하철 2·9호선 도보 0분, 전국 최고 접근성입니다.'},
      {icon:'✦', title:'모듈형 홀', desc:'2~4개 구역 분할로 행사 규모에 맞게 운영 가능합니다.'},
    ],
  ...
  },
  {
    ...
    summary:'명동·시청 도심 중심부로 접근이 용이하며 클래식한 호텔 분위기와 최신 시설을 함께 갖추고 있습니다.',
    tags:['도심','품격','특급','해외vip','뒷풀이'],
    pros:[
      {icon:'✦', title:'도심 최중심', desc:'을지로입구역 바로 인접, 전국 어디서든 접근 용이합니다.'},
      {icon:'✦', title:'자체 조명 시스템', desc:'핀조명·무드·LED 무빙 전 카테고리 자체 보유합니다.'},
    ],
    cautions:[{icon:'△', title:'주차 제한', desc:'도심 특성상 주차가 150대로 제한적입니다.'}],
  ...
  },
  {
    ...
    summary:'테헤란로 랜드마크 38층 전망 연회장으로 강남 파노라마 뷰가 탁월합니다. B2B 기업 행사에 최적화된 입지입니다.',
    tags:['강남','전망','b2b','소규모'],
    pros:[
      {icon:'✦', title:'강남 파노라마', desc:'38층에서 강남 도심 전경을 배경으로 연출 가능합니다.'},
    ],
  ...
  },
  {
    ...
    summary:'한강 수상 위의 독창적 공간으로 강렬한 차별화가 필요한 론칭 행사·시상식에 특히 강한 인상을 남깁니다.',
    tags:['차별화','한강','특수','소규모'],
    pros:[
      {icon:'✦', title:'한국 유일 수상홀', desc:'강렬한 차별화 포인트로 참석자에게 오래 기억됩니다.'},
    ],
...
const APP = {
  step: 1,
  formData: null,
  requestText: '',       // 자유 서술 원문
  parsedRequest: null,   // AI/키워드 분석 결과
  selectedVenue: null,
};
...
/* 글자 수 카운터 */
const reqTextarea = document.getElementById('clientRequest');
const charCount   = document.getElementById('charCount');
if (reqTextarea && charCount) {
  reqTextarea.addEventListener('input', () => {
    charCount.textContent = reqTextarea.value.length;
  });
}
...
function buildCondPills() {
  ...
  document.getElementById('condBar').innerHTML =
    ...
    `<span style="margin-left:auto;padding:3px 10px;background:#EEF2FF;color:#4338CA;border-radius:4px;font-size:10px;font-family:var(--mono);border:1px solid #C7D2FE">✦ AI 분석 모드</span>`;
}

/* ═══════════════════════════════════════
   STEP 2 — 핵심: 요청사항 AI 분석 + 추천
═══════════════════════════════════════ */
...
async function analyzeRequest(text) {
  const SYSTEM = `당신은 행사 기획 전문가입니다. 클라이언트가 자유롭게 쓴 요청 텍스트를 분석하여 아래 JSON만 출력하세요. 마크다운 없이 순수 JSON만.
{
  "keywords": ["추출된 핵심 키워드 3~6개"],
  "requirements": ["명확한 요구사항 문장 2~4개"],
  "concerns": ["우려사항 또는 피해야 할 것 1~3개"],
  "tone": "격식/친근/모던/럭셔리/실용 중 하나",
  "summary": "한 문장으로 요약한 클라이언트 핵심 니즈"
}`;
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 400,
        system: SYSTEM,
        messages: [{ role: 'user', content: `클라이언트 요청사항:\n"${text}"` }],
      }),
    });
    ...
    /* API 실패 시 클라이언트-사이드 키워드 추출로 폴백 */
    return fallbackParse(text);
  }
}

/* ── API 실패 시 클라이언트-사이드 키워드 추출 ── */
function fallbackParse(text) {
  ...
  if (t.includes('주차')) {
    keywords.push('주차');
    const match = text.match(/주차\s*(\d+)대/);
    requirements.push(match ? `주차 ${match[1]}대 이상 확보` : '충분한 주차 공간 필요');
    if (t.includes('불편') && t.includes('주차')) concerns.push('이전 행사에서 주차 불편 경험');
  }
  /* VIP / 품격 */
  if (t.includes('대표') || t.includes('vip') || t.includes('품격') || t.includes('격식')) {
    keywords.push('VIP품격');
    requirements.push('VIP·임원급 참석자를 위한 품격 있는 공간 필요');
  }
  /* 뒷풀이 / F&B */
  if (t.includes('뒷풀이') || t.includes('식사') || t.includes('음식') || t.includes('f&b')) {
    keywords.push('뒷풀이공간');
    requirements.push('행사 후 식음 공간 또는 뒷풀이 장소 동일 건물 내 필요');
  }
  /* 예산 */
  if (t.includes('예산') || t.includes('비싸지 않게') || t.includes('합리적')) {
    keywords.push('예산합리적');
    requirements.push('합리적 예산 범위 내 최적 옵션 선택');
  }
  /* 강남 */
  if (t.includes('강남') || t.includes('역삼') || t.includes('삼성역')) {
    keywords.push('강남권');
  }
  /* 접근성 */
  if (t.includes('교통') || t.includes('대중') || t.includes('접근')) {
    keywords.push('접근성');
    requirements.push('대중교통 접근성 우수한 장소');
  }

  if (!keywords.length) keywords.push('행사 품질', '참석자 편의');
  if (!requirements.length) requirements.push('요청사항에 부합하는 공간');

  return {
    keywords,
    requirements,
    concerns,
    tone: t.includes('럭셔리') || t.includes('프리미엄') ? '럭셔리' : '격식',
...
function scoreAndRank() {
  ...
  const scored = all.map(venue => {
    ...
    /* 기본 조건 반영 */
    if (r.vip_attendance && (venue.tags?.includes('vip') || venue.type?.includes('특급'))) {
      score += 3;
      matchNotes.push('VIP 참석 조건에 적합한 공간');
    }
    if (r.attendees) {
    ...
    if (p) {
      ...
      if (kws.some(k => k.includes('주차')) && venue.tags?.includes('주차여유')) {
        score += 6;
        matchNotes.push('✅ 넉넉한 주차 공간 보유 (요청 반영)');
      }
      if (kws.some(k => k.includes('vip') || k.includes('품격')) && (venue.tags?.includes('vip') || venue.tags?.includes('품격'))) {
        score += 5;
        matchNotes.push('✅ VIP 품격 행사에 최적화된 시설');
      }
      if (kws.some(k => k.includes('뒷풀이')) && venue.tags?.includes('뒷풀이')) {
        score += 4;
        matchNotes.push('✅ 동일 건물 내 식음 공간 연계 가능');
      }
      if (kws.some(k => k.includes('강남')) && venue.tags?.includes('강남')) {
        score += 3;
        matchNotes.push('✅ 강남권 위치');
      }
      if (kws.some(k => k.includes('접근')) && venue.tags?.includes('접근성')) {
        score += 4;
        matchNotes.push('✅ 대중교통 접근성 우수');
      }
      if (kws.some(k => k.includes('차별')) && venue.tags?.includes('차별화')) {
        score += 5;
        matchNotes.push('✅ 독창적인 차별화 공간');
      }
    }

    return {
      ...venue,
      match_percent: Math.min(98, Math.max(58, Math.round(score))),
      matchNotes,
    };
  });
...
function renderRequestAnalysis() {
  ...
  const existing  = document.getElementById('reqAnalysisBox');
  ...
  if (!p) {
    box.innerHTML = `
      <div class="ra-header">
        ...
        <div><div class="ra-title">요청사항 접수됨</div><div class="ra-subtitle">장소 추천에 반영되었습니다</div></div>
      </div>
      <div class="ra-original">"${APP.requestText}"</div>`;
  } else {
    const tagsHTML = (p.keywords || []).map(k => `<span class="ra-tag">${k}</span>`).join('');
    const reqsHTML = (p.requirements || []).map(r => `<li style="margin-bottom:4px">${r}</li>`).join('');
    const consHTML = (p.concerns || []).length > 0
      ? `<div style="margin-top:10px;font-size:12px;color:#92400E;background:#FFFBEB;border:1px solid #FDE68A;border-radius:6px;padding:8px 12px">
          ⚠ 피해야 할 것: ${p.concerns.join(' / ')}</div>`
      : '';
    ...
    box.innerHTML = `
      <div class="ra-header">
        ...
        <div>
          ...
          <div class="ra-subtitle">${p.summary || '핵심 니즈가 추천에 반영되었습니다'}</div>
        </div>
      </div>
      <div class="ra-original">"${APP.requestText}"</div>
      <div class="ra-tags-label">추출된 핵심 키워드</div>
      <div class="ra-tags">${tagsHTML}</div>
      ${reqsHTML ? `<ul style="margin-top:12px;font-size:12px;color:#4338CA;padding-left:16px;line-height:1.8">${reqsHTML}</ul>` : ''}
      ${consHTML}`;
  }
...
function renderVenues(venues) {
  ...
  venues.forEach(v => {
    ...
    /* 요청 반영 배너 */
    let matchBanner = '';
    if (v.matchNotes && v.matchNotes.length > 0) {
      const allMet = v.matchNotes.every(n => n.startsWith('✅'));
      matchBanner = `
        <div class="req-match-banner ${allMet ? '' : 'partial'}">
          <span>${allMet ? '🎯' : '📋'}</span>
          <div>
            <strong>클라이언트 요청 반영</strong><br>
            ${v.matchNotes.join(' · ')}
          </div>
        </div>`;
    }
...
function openAdmin() {
  ...
  panel.innerHTML = `<style>
    ...
    .pi input:focus{border-color:#1A1A1A}.pic{text-align:center}
  ...
  <div class="am">
    ...
    <div class="am-body">
      <div class="tp on" id="tp-add">
        ...
        <div class="gr2">
          <div class="fg"><label>장소명 *</label><input id="an" type="text" placeholder="장소명 입력"></div>
          <div class="fg"><label>장소 유형 *</label>
            <select id="at"><option value="">선택</option><option>특급호텔 연회장</option><option>비즈니스호텔 연회장</option><option>전문 컨벤션센터</option><option>프리미엄 오피스빌딩 연회장</option><option>특수 이벤트홀</option><option>기타</option></select>
          </div>
        </div>
        <div class="gr2">
        ...
        <div class="gr1"><div class="fg"><label>한줄 요약</label><textarea id="asum" placeholder="추천 이유 2~3문장"></textarea></div></div>
        ...
        <div id="proList"><div class="pi"><input class="pic" type="text" value="✦"><input type="text" placeholder="강점 제목"><input type="text" placeholder="설명"><button class="rm" onclick="this.closest('.pi').remove()">✕</button></div></div>
        <button class="addbtn" id="addPro">+ 강점 추가</button>
        <div class="sec">주의사항</div>
        <div id="cautList"><div class="pi"><input class="pic" type="text" value="△"><input type="text" placeholder="주의사항 제목"><input type="text" placeholder="설명"><button class="rm" onclick="this.closest('.pi').remove()">✕</button></div></div>
  ...
  function addRow(lid, ico, p1, p2) {
    const r = document.createElement('div'); r.className = 'pi';
    r.innerHTML = `<input class="pic" type="text" value="${ico}"><input type="text" placeholder="${p1}"><input type="text" placeholder="${p2}"><button class="rm" onclick="this.closest('.pi').remove()">✕</button>`;
    document.getElementById(lid).appendChild(r);
  }
  document.getElementById('addPro').addEventListener('click', () => addRow('proList','✦','강점 제목','설명'));
  document.getElementById('addCaut').addEventListener('click', () => addRow('cautList','△','주의사항','설명'));
  ...
  document.getElementById('amSave').addEventListener('click', () => {
    ...
    const pros = [...document.querySelectorAll('#proList .pi')].map(r => {
      ...
      return { icon: i[0].value||'✦', title: i[1].value, desc: i[2].value };
    }).filter(p => p.title);
    ...
    saveCustomVenues([...getCustomVenues(), {
      ...
      pros, cautions, tags:[],
      specs:{ capacity:document.getElementById('sc').value, area:document.getElementById('sa').value, ceiling:document.getElementById('sh').value, parking:document.getElementById('sp').value, av:document.getElementById('sv').value, catering:document.getElementById('sk').value },
    }]);
    closeAdmin();
  ...
  function renderAdmList() {
    ...
    box.innerHTML = all.map(v=>`<div class="vc-li"><div><div class="vc-li-name">${v.name}</div><div class="vc-li-meta">${v.type} · ${v.location||'주소미입력'} · ${v._b?'<span style="color:#B8860B">내장</span>':'<span style="color:#10B981">직접등록</span>'}</div></div>${!v._b?`<button class="delbtn" data-id="${v.id}">삭제</button>`:'<span style="font-size:10px;color:#ABA99F">내장</span>'}</div>`).join('');
    box.querySelectorAll('.delbtn').forEach(btn => {
      btn.addEventListener('click', () => { saveCustomVenues(getCustomVenues().filter(v=>v.id!==btn.dataset.id)); renderAdmList(); showToast('삭제됨'); });
    });
  }
}
